...
import FairwayService from "../../database/zone/fairway/fairway.service";
import { fairwayInfoEx } from "../../models/examples/zone/fairwayInfoEx";
...
export default class FairwayController {
	...
	async getClubFairway(@Param('siteid') siteid:string, @Headers('WWW-AUTH') token:Record<string, string>){
		const resp = await this.query(String(token), siteid);
		return resp;
	}
	...
	@ApiResponse({status: 200, description:'回傳資料', type: fairwayInfoResponse})
	async getZonefairway(@Param('siteid') siteid:string, @Param('zoneid') zoneid:string, @Headers('WWW-AUTH') token:Record<string, string>){
		const resp = await this.query(String(token), siteid, zoneid);
		return resp;
	}	
	...
	@ApiResponse({status: 200, description:'回傳資料', type: fairwayInfoResponse})
	async getfairway(@Param('siteid') siteid:string, @Param('zoneid') zoneid:string, @Param('fairwayid') fairwayid:string, @Headers('WWW-AUTH') token:Record<string, string>){
		const fwayid = fairwayid ? Number(fairwayid) : 0;
		const resp = await this.query(String(token), siteid, zoneid, fwayid);
		return resp;
	}
	...
	*/
	async query(token:string, siteid:string, zoneid?:string, fairwayid?:number) {
		let resp:commonResWithData<fairwayInfo[]> = {
			errcode: '0',
		}
		const user = tokenCheck(String(token));
		if (user) {
			if (siteid && isMyClub(user, siteid)) {
				try {			
